<?php
	...
	function cedulaExiste($cedula){
		...
		$stmt = $mysqli->prepare("SELECT id FROM usuarios WHERE cedula = ? LIMIT 1");
	...
	function existente($nombre, $apellido){

		global $mysqli;
		$stmt = $mysqli->prepare("SELECT id FROM usuarios WHERE nombre = ? AND apellido = ? LIMIT 2");
	...
	function registraUsuario($usuario, $pass_hash, $cedula, $nombre, $apellido, $personal, $cargo, $vicerrectorado, $nucleo, $telefono, $email, $activo, $token, $tipo_usuario){
		
		global $mysqli;
		
		$stmt = $mysqli->prepare("INSERT INTO usuarios (usuario, password, cedula, nombre, apellido, personal, cargo, vicerrectorado, nucleo, telefono, correo, activacion, token, id_tipo) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$stmt->bind_param('ssissssssisisi', $usuario, $pass_hash, $cedula, $nombre, $apellido, $personal, $cargo, $vicerrectorado, $nucleo, $telefono, $email, $activo, $token, $tipo_usuario);
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Username = 'sercomweb2017@gmail.com';
		$mail->Password = 'holaquehay1';
		
		$mail->setFrom('sercomweb2017@gmail.com', 'Sistema de Usuarios');
